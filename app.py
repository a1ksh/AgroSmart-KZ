...
def hectares_to_radius_m(area_ha: float) -> float:
    area_m2 = max(float(area_ha), 0.0) * 10_000.0
    return math.sqrt(area_m2 / math.pi) if area_m2 > 0 else 0.0


def find_nearest_region(lat: float, lon: float) -> str:
    best, best_dist = None, float("inf")
    for region_name, info in KZ_REGIONS.items():
        d = math.sqrt((float(info["lat"]) - float(lat)) ** 2 + (float(info["lon"]) - float(lon)) ** 2)
        if d < best_dist:
            best_dist = d
            best = region_name
...
st.markdown(
    """
<style>
*, *::before, *::after { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif !important; box-sizing: border-box; }
.icon-asset {
    width: 18px;
    height: 18px;
    display: inline-flex;
...
div[data-testid="stDateInput"] div[data-testid="stWidgetLabel"] {
    color: #1f2937 !important;
    visibility: visible !important;
    opacity: 1 !important;
}

/* ── ВЕРИФИКАЦИЯ ФОРМАСЫ: LABEL КӨРІНУІ ── */
...
/* Help иконкасы */
button[data-testid="stWidgetLabelHelpButton"] {
    color: #6b7280 !important;
}

/* Sidebar * { color } ережесі label-ды жаппасын */
...
/* Nav overlay buttons — мөлдір, HTML div үстінде орналасады */
...
/* Hover: кнопка үстінде тұрғанда — алдыңғы div-ді жасылдат */
section[data-testid="stSidebar"] div[data-testid="stMarkdownContainer"]:has(+ div[data-testid="stButton"] button:hover) > div {
...
def icon_svg(name: str) -> str:
    icons = {
        ...
        "arrow_right": '<svg viewBox="0 0 24 24" aria-hidden="true"><polyline points="9 18 15 12 9 6"></polyline></svg>',
        "arrow_down": '<svg viewBox="0 0 24 24" aria-hidden="true"><polyline points="6 9 12 15 18 9"></polyline></svg>',
        "arrow_up": '<svg viewBox="0 0 24 24" aria-hidden="true"><polyline points="18 15 12 9 6 15"></polyline></svg>',
        "arrow_left": '<svg viewBox="0 0 24 24" aria-hidden="true"><polyline points="15 18 9 12 15 6"></polyline></svg>',
    }
    if name not in icons:
        fallback = {"arrow_right": "→", "arrow_down": "↓", "arrow_up": "↑", "arrow_left": "←"}
        return f"<span style='font-size:14px'>{fallback.get(name, '•')}</span>"
    return f'<span class="icon-asset">{icons[name]}</span>'
...
nav_items = [
    ...
    "Болжамдау",
    "Жер талдауы",
    "Субсидия верификациясы",
]

if "page" not in st.session_state:
    st.session_state["page"] = "Жалпы шолу"
...
<div style="padding:16px 8px 8px;">
  <div style="font-size:20px;font-weight:800;color:#0f8a45;">
    AgroSmart KZ
  </div>
  <div style="font-size:11px;color:#64748b;margin-top:2px;">
    Мемлекеттік аграр портал
  </div>
</div>
<hr style="border:none;border-top:1px solid #e5ebe7;margin:8px 0;">
    """,
        unsafe_allow_html=True,
    )
        ...
        if is_active:
            st.markdown(
                f"""<div style="
                    ...
                    background:#0f8a45; color:#ffffff;
    ...
    st.markdown(
        """
<hr style="border:none;border-top:1px solid #e5ebe7;margin:16px 0;">
<div style="padding:0 4px;">
  <div style="padding:9px 14px;background:#f0fdf4;color:#0f8a45;
       border-radius:10px;font-size:13px;font-weight:600;
       margin-bottom:6px;cursor:pointer;">
       Есепті жүктеу
  </div>
  <div style="padding:9px 14px;background:#f8fafc;color:#64748b;
       border-radius:10px;font-size:13px;margin-bottom:6px;
       cursor:pointer;">
       Параметрлер
  </div>
  <div style="padding:9px 14px;background:#fff1f2;color:#dc2626;
    ...
    with left_col:
        ...
        if result is not None:
            ...
            with cl:
                ...
                crops_html = " ".join(
                    [
                        f"<span style='background:#f1f5f9;color:#111827;border:1px solid #d1d5db;padding:4px 10px;border-radius:6px;font-size:12px;font-weight:600;'>{c}</span>"
            ...
            with cr:
                ...
                recs_html = "".join(
                    [
                        f"<div style='padding:5px 0; border-bottom:1px solid #edf2ef; font-size:13px; color:#111827;'>{r}</div>"
...
# ═══════════════════════════════════════════════════════════════
    ...
    if "auto_region" not in st.session_state:
        st.session_state["auto_region"] = oblasts_14[0]
    if "map_pick_lat" not in st.session_state:
        st.session_state["map_pick_lat"] = float(st.session_state["subsidy_lat"])
    if "map_pick_lon" not in st.session_state:
        st.session_state["map_pick_lon"] = float(st.session_state["subsidy_lon"])

    def detect_region(lat, lon):
        bounds = {
            "Ақмола": (50.5, 53.5, 67.0, 75.0), "Ақтөбе": (47.5, 51.5, 55.0, 64.0),
            "Алматы": (42.5, 45.5, 75.0, 84.0), "Атырау": (45.5, 48.5, 51.0, 57.0),
            "Шығыс Қазақстан": (47.0, 50.5, 79.0, 87.5), "Жамбыл": (42.0, 45.0, 69.0, 76.0),
            "Батыс Қазақстан": (48.5, 52.0, 49.0, 56.0), "Қарағанды": (46.0, 51.0, 68.0, 79.0),
            "Қостанай": (51.5, 55.0, 60.0, 69.0), "Қызылорда": (43.5, 47.5, 60.0, 68.0),
            "Маңғыстау": (43.0, 46.5, 51.0, 56.0), "Павлодар": (50.5, 54.0, 74.0, 79.5),
            "Солтүстік Қазақстан": (53.0, 55.5, 66.0, 75.0), "Түркістан": (41.0, 44.0, 66.0, 72.0),
        }
        for name, (a, b, c, d) in bounds.items():
    ...
    with col_form:
        ...
        st.markdown("<p class='gov-field-label'>Аты-жөні</p>", unsafe_allow_html=True)
        st.markdown("<p class='gov-field-help'>Субсидия алушының толық аты-жөнін енгізіңіз</p>", unsafe_allow_html=True)
        farmer_name = st.text_input(
            "Аты-жөні",
            value="",
            placeholder="Мысалы: Ахметов Берік Сейткалиұлы",
            help="Субсидия алушының толық аты-жөні",
            label_visibility="collapsed",
        )

        auto_region = detect_region(
            st.session_state["subsidy_lat"],
            st.session_state["subsidy_lon"]
        )
        default_idx = oblasts_14.index(auto_region) if auto_region in oblasts_14 else 0
        st.markdown("<p class='gov-field-label'>Облыс</p>", unsafe_allow_html=True)
        st.markdown("<p class='gov-field-help'>Тізімнен таңдаңыз</p>", unsafe_allow_html=True)
        region = st.selectbox(
            "Облыс",
            oblasts_14,
            index=default_idx,
            help="Жер учаскесі орналасқан облыс. Картадан нүкте бассаңыз — автоматты анықталады",
            label_visibility="collapsed",
        )

        st.markdown("<p class='gov-field-label'>Дақыл түрі</p>", unsafe_allow_html=True)
        st.markdown("<p class='gov-field-help'>Тізімнен таңдаңыз</p>", unsafe_allow_html=True)
        crop_type = st.selectbox(
            "Дақыл түрі",
            crops,
            help="Субсидияланатын негізгі дақыл түрін таңдаңыз",
            label_visibility="collapsed",
        )

        st.markdown("<p class='gov-field-label'>Жер ауданы (га)</p>", unsafe_allow_html=True)
        st.markdown("<p class='gov-field-help'>Гектармен енгізіңіз</p>", unsafe_allow_html=True)
        ...
        st.markdown("<p class='gov-field-label'>Сұралған субсидия сомасы (₸)</p>", unsafe_allow_html=True)
        st.markdown("<p class='gov-field-help'>Бидай: ~15 000–25 000 ₸/га | Рапс: ~18 000–30 000 ₸/га</p>", unsafe_allow_html=True)
        subsidy_amount = st.number_input(
            "Сұралған субсидия сомасы (₸)",
            min_value=0.0,
            value=5_000_000.0,
            step=100_000.0,
            help="Бидай: ~15 000–25 000 ₸/га | Рапс: ~18 000–30 000 ₸/га",
            label_visibility="collapsed",
        )

        st.markdown("<p class='gov-field-label'>Өтінім берілген күн</p>", unsafe_allow_html=True)
        st.markdown("<p class='gov-field-help'>Субсидия өтінімінің ресми тіркелген күні</p>", unsafe_allow_html=True)
        application_date = st.date_input(
            "Өтінім берілген күн",
            help="Субсидия өтінімінің ресми тіркелген күні",
            label_visibility="collapsed",
        )

        st.markdown(
            "<p class='gov-field-label'>Координаттары</p>",
            unsafe_allow_html=True
        )
        st.caption("Картадан нүкте басыңыз — автоматты толтырылады.")
        _c1, _c2 = st.columns(2)
        with _c1:
            st.markdown("<p class='gov-field-label'>Ендік (Latitude °N)</p>", unsafe_allow_html=True)
            st.markdown("<p class='gov-field-help'>Солтүстік ендік. Қазақстан: 40°N – 55°N</p>", unsafe_allow_html=True)
            latitude = st.number_input(
                "Ендік (Latitude °N)",
                min_value=40.0, max_value=56.0,
                value=float(st.session_state["subsidy_lat"]),
                format="%.6f",
                help="Солтүстік ендік. Қазақстан: 40°N – 55°N",
                label_visibility="collapsed",
            )
        with _c2:
            st.markdown("<p class='gov-field-label'>Бойлық (Longitude °E)</p>", unsafe_allow_html=True)
            st.markdown("<p class='gov-field-help'>Шығыс бойлық. Қазақстан: 46°E – 87°E</p>", unsafe_allow_html=True)
            longitude = st.number_input(
                "Бойлық (Longitude °E)",
                min_value=46.0, max_value=88.0,
                value=float(st.session_state["subsidy_lon"]),
                format="%.6f",
                help="Шығыс бойлық. Қазақстан: 46°E – 87°E",
                label_visibility="collapsed",
            )

        # Қолмен енгізілген координаттарды сессияда сақтаймыз.
        st.session_state["subsidy_lat"] = latitude
        st.session_state["subsidy_lon"] = longitude

        if st.button("Картадан автоматты толтыру", key="subsidy_autofill_btn", use_container_width=True):
            # Картадан таңдалған нүктені формаға қоямыз, басқа өрістер өзгермейді.
            st.session_state["subsidy_lat"] = float(st.session_state["map_pick_lat"])
            st.session_state["subsidy_lon"] = float(st.session_state["map_pick_lon"])
            st.session_state["auto_region"] = detect_region(
                st.session_state["subsidy_lat"],
                st.session_state["subsidy_lon"],
            )
            st.rerun()
        ...
        _norms = {
            "Бидай": ("15 000–25 000", "🌾"), "Арпа": ("12 000–20 000", "🌿"),
            "Рапс": ("18 000–30 000", "🟡"), "Күнбағыс": ("14 000–22 000", "🌻"),
            "Картоп": ("20 000–35 000", "🥔"), "Жүзім": ("25 000–50 000", "🍇"),
            "Басқа": ("10 000–20 000", "🌱")
        }
        _norm, _emoji = _norms.get(crop_type, ("10 000–20 000", "🌱"))
        _est = declared_area_ha * 20_000
        st.markdown(
            f"""
       <div style='background:#eaf8ef;border-left:4px solid #0f8a45;
           border-radius:8px;padding:10px 14px;margin:6px 0 12px;'>
         <span style='color:#0f8a45;font-weight:700;'>{_emoji} {crop_type}</span>
         <span style='color:#374151;font-size:13px;'> үшін норма:
           <strong>{_norm} ₸/га</strong> ·
           Сіздің аудан: <strong>{declared_area_ha:.1f} га</strong> ·
           Болжамды сома: <strong>₸{_est:,.0f}</strong></span>
       </div>""",
            unsafe_allow_html=True
        )
    ...
    with col_map:
        ...
        declared_radius_m = hectares_to_radius_m(declared_area_ha)
        fmap = folium.Map(
        ...
        if declared_radius_m > 0:
            folium.Circle(
                location=[float(latitude), float(longitude)],
                radius=declared_radius_m,
                color="#2563eb",
                weight=2,
                fill=True,
                fill_opacity=0.15,
                tooltip=f"Өтінім аумағы: {declared_area_ha:.2f} га",
            ).add_to(fmap)
        ...
        selected_lat = float(st.session_state["subsidy_lat"])
        selected_lon = float(st.session_state["subsidy_lon"])
        last_clicked = map_state.get("last_clicked") if isinstance(map_state, dict) else None
        if last_clicked:
            # Картадан click болған координатты уақытша сақтаймыз.
            st.session_state["map_pick_lat"] = float(last_clicked["lat"])
            st.session_state["map_pick_lon"] = float(last_clicked["lng"])
            st.success(
                f"Карта нүктесі таңдалды: {float(st.session_state['map_pick_lat']):.6f}, "
                f"{float(st.session_state['map_pick_lon']):.6f}"
            )
        else:
            st.caption("Картадан нүкте бассаңыз, координат автоматты жаңарады.")
        st.markdown(
            f"<span class='coord-pill'>Таңдалған координат: {float(st.session_state['map_pick_lat']):.6f}, {float(st.session_state['map_pick_lon']):.6f}</span>",
            unsafe_allow_html=True,
        )
  ...
  <div style="background:#eaf8ef; border-left:4px solid #0f8a45;
      ...
      <strong>ℹ️ Верификация туралы ақпарат</strong><br>
      <span style="font-size:13px; color:#374151;">
      Жүйе 7 қадамда тексереді: координаттар → спутниктік сурет →
      дақыл анықтау → аудан есептеу → денсаулық бағасы → скоринг → шешім.<br>
      <strong>Орташа уақыт: 3–5 секунд.</strong>
      </span>
  </div>
  """,
        unsafe_allow_html=True,
    ...
    if verify_btn:
        ...
        st.session_state["subsidy_last_result"] = result
        st.session_state["subsidy_result_lat"] = selected_lat
        st.session_state["subsidy_result_lon"] = selected_lon

    result = st.session_state.get("subsidy_last_result")
    result_lat = float(st.session_state.get("subsidy_result_lat", selected_lat))
    result_lon = float(st.session_state.get("subsidy_result_lon", selected_lon))
    ...
    if result:
        ...
        with st.expander("Балл бөлінісі", expanded=True):
            ...
            for title, key, max_points in labels:
                ...
                st.markdown(
                    f"<p style='color:#1f2937; font-weight:700; margin:6px 0;'>{title}: "
                    f"<span style='color:#0f8a45;'>{points}</span>/{max_points}</p>",
                    unsafe_allow_html=True
                )
...
div[data-testid="stJson"] span { color: #1f2937 !important; }
div[data-testid="stJson"] .string { color: #0f8a45 !important; }
div[data-testid="stJson"] .number { color: #1a5276 !important; }
div[data-testid="stJson"] .boolean { color: #922b21 !important; }
</style>
""", unsafe_allow_html=True)
            st.json(result["satellite"])

        with st.expander("Егістік аумағы (га)"):
            area_map = folium.Map(
            ...
            folium.Marker(
                ...
                tooltip="Орталық нүкте",
                icon=folium.Icon(color="green", icon="ok-sign"),
            ).add_to(area_map)

            declared_area = float(details.get("declared_area_ha", 0.0))
            detected_area = float(details.get("detected_area_ha", 0.0))
            declared_radius = hectares_to_radius_m(declared_area)
            detected_radius = hectares_to_radius_m(detected_area)

            if declared_radius > 0:
                folium.Circle(
                    location=[result_lat, result_lon],
                    ...
                    fill=True,
                    fill_opacity=0.12,
                    tooltip=f"Өтінім: {declared_area:.2f} га",
                ).add_to(area_map)
            ...
            if detected_radius > 0:
                folium.Circle(
                    ...
                    fill=True,
                    fill_opacity=0.18,
                    tooltip=f"Анықталған: {detected_area:.2f} га",
                ).add_to(area_map)

            st_folium(
                area_map,
                key="subsidy_result_area_map",
                use_container_width=True,
                height=420,
            )
